...
class TimeReportForDayCommand(Command):
    ...
    def run(self, ctx: Program) -> CommandCode:
        ...
        todays_courses = sum(
            [
                (
                    datetime.strptime(str(sch[2]), "%H:%M:%S")
                    - datetime.strptime(str(sch[1]), "%H:%M:%S")
                ).total_seconds()
                / 3600
        ...
        users_activities = [
            a for a in ctx.manager.activities if a.belongs_to == ctx.user.id
        ]
        todays_activities = sum(
            [
                (
                    datetime.strptime(str(activity.end_hour), "%H:%M:%S")
                    - datetime.strptime(str(activity.start_hour), "%H:%M:%S")
                ).total_seconds()
                / 3600
