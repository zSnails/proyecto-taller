...
class CoursesTab(Frame):
    ...
    def new_course_tab(self):
        ...
        course_hours = 0
        self.weekly_hours = 0
        weekdays_list = ["None"] + [str(day).split(".").pop() for day in WeekDays]
        ...
        def register_course():
            ...
            # start_date = datetime.strptime(start_date_calendar.get_date(), "%Y/%m/%d")
            # end_date = datetime.strptime(end_date_calendar.get_date(), "%Y/%m/%d")
            course_duration = (
